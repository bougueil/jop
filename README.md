...
<!-- MDOC !-->
Jop logs in memory, spatially and temporally, key value events.

- A Jop instance can be shared during logging by multiple processes.
- Several Jop instances can be started and run concurrently.

These events, generated by multiple processes, are then flushed to disk for analysis (e.g. to detect locks).

> #### Warning {: .warning}
>
> Since Jop uses the memory while logging, its usage should be reserved for short time investigations.

## Usage
```
  iex> "myjop"
  ...> |> Jop.init() # init the instance "myjop"
